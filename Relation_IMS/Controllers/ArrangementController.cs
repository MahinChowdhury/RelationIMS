...
namespace Relation_IMS.Controllers
{
    ...
    public class ArrangementController : ControllerBase
    {
        ...
        {
            // CRITICAL: Dual-locking to prevent same item from being assigned to multiple orders
            // Lock product item FIRST (by barcode), then order
            using (await _lockService.AcquireLockAsync($"productitem:{request.Barcode}"))
